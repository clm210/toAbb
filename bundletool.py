...
def task(task_name, fun, *args, **kwargs):
    ...
    print_log("###耗时:{} {} status{} msg{}".format(end_time - start_time,task_name,status,msg))
    if status != 0:
        #raise Exception(f"task {task_name} 执行异常status:{status} msg:{msg}")
        raise Exception("task {} 执行异常status:{} msg:{}".format(task_name,status,msg))


def compile_resources(compile_source_res_dir, compiled_resources, aapt2):
    """
    编译 res目录
    ...
    cmd = "{} compile --legacy\
        --dir {} \
        -o {} ".format(aapt2,compile_source_res_dir,compiled_resources)
    return execute_cmd(cmd)


def link_asset_resources(link_out_apk_path,
                         input_manifest,
                         android,
                         aapt2):
    """
    ...
    # cmd = f"{aapt2} link --proto-format \
        ...
        # --auto-add-overlay"
    cmd = "{} link --proto-format \
        -o {} \
        -I {} \
        --manifest {} \
        --auto-add-overlay".format(aapt2,link_out_apk_path,android,input_manifest)
    return execute_cmd(cmd)


def link_resources(link_out_apk_path,
                   input_manifest,
                   android,
                   min_sdk_version,
                   target_sdk_version,
    ...
    cmd = "{} link --proto-format \
        ...
        --version-name {}\
        --manifest {} \
        --auto-add-overlay".format(aapt2,link_out_apk_path,android,min_sdk_version,target_sdk_version,version_code,version_name,input_manifest)

    if compiled_resources_path and os.path.exists(compiled_resources_path):
        cmd += " -R {}".format(compiled_resources_path)
    if public_id_path and os.path.exists(public_id_path):
        cmd += " --stable-ids {}".format(public_id_path)
    return execute_cmd(cmd)
...
def build_bundle(bundletool, modules, out_aab_path, bundle_config_json_path):
    """
    构建aab
    ...
    """
    cmd = "java -jar {} build-bundle \
        --modules={} \
        --output={} ".format(bundletool,modules,out_aab_path)
    if bundle_config_json_path and os.path.exists(bundle_config_json_path):
        cmd += " --config={}".format(bundle_config_json_path)
    return execute_cmd(cmd)


def decode_apk(apk_path, decode_apk_dir, apktool):
    cmd = "java -jar {} d {} -s -o {}".format(apktool,apk_path,decode_apk_dir)
    return execute_cmd(cmd)
...
def create_pad_module_dir(temp_dir, module_name, package):
    ...
    text = utils.read_file_text(template_android_manifest_path) \
...
def sign(temp_aab_path, keystore, storepass, keypass, alias):
    ...
    #     {temp_aab_path} \
    #     {alias}"
    cmd = "jarsigner -digestalg SHA1 -sigalg SHA1withRSA \
        -keystore {} \
        -storepass {} \
        -keypass {} \
        {} \
        {}".format(keystore,storepass,keypass,temp_aab_path,alias)

    return execute_cmd(cmd)


def create_bundle_config_json(bundle_config_json_path, do_not_compress):
...
class Bundletool:
    ...
    def check_system(self, apk_path, out_aab_path):
        print_log("[当前系统]:{}".format(get_system()))
        print_log("[当前系统JAVA版本]↓↓↓↓↓:")
        _, msg = execute_cmd("java -version")
        print_log("[输入apk]:{}".format(apk_path))
        if not os.path.exists(apk_path):
            return -1, "输入的apk不存在:{}".format(apk_path)
        print_log("[输出aab]:{}".format(out_aab_path))
        print_log("[签名]:{},storepass:{},alias:{},keypass:{}".format(self.keystore,self.storepass,self.alias,self.keypass))
        if not os.path.exists(self.keystore):
            return -2, "输入的keystore不存在:{}".format(self.keystore)
        status, msg = execute_cmd(
            "keytool -list -v -keystore {} -storepass {} -alias {} ".format(self.keystore,self.storepass,self.alias))
        status += status
        print_log("↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓")
        if get_system() in [MACOS, Linux]:
            status, msg = execute_cmd(
                "keytool -exportcert -alias {} -keystore {} -storepass {} | openssl sha1 -binary | openssl base64".format(self.alias,self.keystore,self.storepass))
            if status != 0:
                return -999, "签名错误"
        else:
            print_log("window不去校验，避免没有openssl的库")
        print_log("↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑")
        print_log("[apktool]:{}".format(self.apktool))
        print_log("[apktool版本号]:↓↓↓↓↓")
        status, msg = execute_cmd("java -jar {} --version".format(self.apktool))
        status += status
        # 如果是linux 或者 mac 需要给aapt可执行权限
        if get_system() in [MACOS, Linux]:
            try:
                execute_cmd("chmod +x {}".format(self.aapt2))
            except Exception as e:
                print_log("授权失败:", e)
                pass
            pass
        print_log("[aapt2]:{}".format(self.aapt2))
        print_log("[aapt2版本号]:↓↓↓↓↓")
        status, msg = execute_cmd("{} version".format(self.aapt2))
        status += status
        print_log("[android]:{}".format(self.android))
        if not os.path.exists(self.android):
            return -3, "输入的android.jar不存在:f{}".format(self.android)
        print_log("[bundletool]:{}".format(self.bundletool))
        print_log("[bundletool版本号]:↓↓↓↓↓")
        status, msg = execute_cmd("java -jar {} version".format(self.bundletool))
        print("cmd 执行完成")
        status += status
    ...
    def build_asset_pack(self, temp_dir, module_name, input_resources_dir, out_module_zip_path):
        # 构建的临时目录
        module_temp_dir = os.path.join(temp_dir, "{}_temp".format(module_name))
        # 创建一下
        os.makedirs(module_temp_dir)
        # 生成的临时apk
        link_base_apk_path = os.path.join(module_temp_dir, "{}.apk".format(module_name))
        # 解压临时apk后的文件夹路径
        ...
        # 构建
        task("[{}]-asset-关联资源".format(module_name), link_asset_resources, link_base_apk_path, input_manifest, self.android,
             self.aapt2)
        # 解压
        task("[{}]-asset-解压resources_apk".format(module_name), unzip_file, link_base_apk_path, unzip_link_apk_path)
        # 拷贝
        task("[{}]-asset-移动AndroidManifest".format(module_name), mv, temp_android_manifest_path, target_manifest_path)
        # 删除
        task("[{}]-asset-删除AndroidManifest".format(module_name), delete, input_manifest)
    ...
    def build_module_zip(self, temp_dir, module_name, input_resources_dir, out_module_zip_path,
                         public_id_path):
        """
        ...
        if os.path.exists(input_res_dir):
            try:
                ...
                # 文件怎么来的？
                task("[{}]-编译资源".format(module_name), compile_resources, input_res_dir, compiled_resources, self.aapt2)
            except Exception as e:
                print_log("[{}]-编译资源错误 {}".format(module_name,str(e)))
                pass
        # 2. 通过 compiled_resources.zip 和 AndroidManifest.xml 生成中间产物 base.apk
        task("[{module_name}]-关联资源", link_resources, link_base_apk_path, input_manifest, self.android,
        ...
        task("[{}]-解压resources_apk".format(module_name), unzip_file, link_base_apk_path, unzip_link_apk_path)
        # 4. 修改AndroidManifest.xml 的 位置 构建aab需要的目录
        task("[{}]-移动AndroidManifest".format(module_name), mv, temp_android_manifest_path, target_android_manifest_path)
        # 5. 拷贝assets
        if os.path.exists(input_assets):
            task("[{}]-拷贝assets".format(module_name), copy, input_assets, target_assets_path)
        # 6. 拷贝lib
        if os.path.exists(input_lib):
            task("[{}]-拷贝lib".format(module_name), copy, input_lib, target_lib_path)
        # 7. 拷贝其他的文件
        if os.path.exists(input_unknown):
            task("[{}]-拷贝unknown".format(module_name), copy, input_unknown, target_unknown_path)
        # 8. 拷贝kotlin的文件
        if os.path.exists(input_kotlin):
            task("[{}]-拷贝kotlin".format(module_name), copy, input_kotlin, target_kotlin_path)
        # 9. 删除apk的签名信息
        if os.path.exists(input_meta_inf_path):
            task("[{}]-处理原有的apk签名信息".format(module_name), delete_sign, input_meta_inf_path)
        # 10. 拷贝META-INF的时候需要先删除 apk的签名信息
        if os.path.exists(input_meta_inf_path):
            task("[{}]-拷贝META-INF".format(module_name), copy, input_meta_inf_path, target_mata_inf_path)
        # 11. 拷贝 dex
        if os.path.exists(input_resources_dir):
            task("[{}]-拷贝dex".format(module_name), copy_dex, input_resources_dir, target_dex_path)
        task("[{}]-拷贝其他文件".format(module_name), copy_other, input_resources_dir, target_unknown_path)
        # 12. 压缩成base.zip
        task("[{}]-压缩zip".format(module_name), zip_file, unzip_link_apk_path, out_module_zip_path)
        return 0, "success"
    ...
    def run(self, apk_path, out_aab_path, pad_reg=""):
        ...
        # 生成临时的工作目录
        #temp_dir = "temp_{}".format(datetime.datetime.now())
        temp_dir = "temp_22222"
        ...
        try:
            ...
            for name, path in self.bundle_modules.items():
                task("[{name}]-构建module压缩包", self.build_module_zip, temp_dir, name, path,
                     os.path.join(module_zip_dir, name + ".zip"), public_id_path)

            for name, path in self.bundle_asset_pack_modules.items():
                task("[{name}]-构建asset_pack_module", self.build_asset_pack, temp_dir, name, path,
            ...
            for name, path in self.bundle_asset_pack_modules.items():
                task("[{name}]-压缩asset_pack in aab", zip_file, path, temp_aab_path, name)
