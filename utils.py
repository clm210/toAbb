...
def read_file_text(file_path):
    print("进入====read_file_text "+file_path)
    with open(file_path, "r") as f:
        return f.read()


def write_file_text(file_path, text):
...
def zip_file(src_dir, zip_name="", parent_dir_name=""):
    ...
    if "w" == mode and parent_dir_name == "":
        # 尝试调用一下系统的压缩方法，  速度快一点。。。
        cmd =  "cd {} && zip -r -q -D {} *".format(src_dir,os.path.abspath(zip_name))
        #cmd = "cd {} && zip -r -q -D {os.path.abspath()} *".format(src_dir,zip_name)
